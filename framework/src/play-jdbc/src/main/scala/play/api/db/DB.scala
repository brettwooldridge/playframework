...
trait DBApi {
  ...
  def getConnectionAsync(name: String, autocommit: Option[Boolean] ) : Future[Connection] = {
    val p = promise[Connection]
    Future {
      blocking {
        try {
          val connection = getDataSource(name).getConnection
          if (autocommit.isDefined) connection setAutoCommit(autocommit.get)
...
object DB {
  ...
  /**
   * Retrieves a JDBC connection future.
   *
   * @param name data source name
   * @param autocommit optional parameter, when undefined the DataSource default is used
   * @return a JDBC connection future
   * @throws an error if the required data source is not registered
   */
  def getConnectionAsync(name: String = "default", autocommit: Option[Boolean])(implicit app: Application): Future[Connection] = app.plugin[DBPlugin].map(_.api.getConnectionAsync(name, autocommit)).getOrElse(error)
